...
`case` — `upper` or `lower`. Governs how the prefix is written in filenames and headings. Do not mix cases within a series.

`heading_form` — one of `dotted`, `undotted`, or `omit`. Controls whether the ID appears in the H2 heading of the decision file and in what shape. `dotted` means the ID is printed as `A.2` in the heading; `undotted` means `A2`; `omit` means the ID is in the filename only and does not appear in the heading at all.

`filename_form` — almost always `undotted`. The filename rarely uses the dotted form because dots confuse some filesystems and tooling.

`filename_pattern_closed` / `filename_pattern_pending` — the template for closed and pending filenames. Supported placeholders: `{date}` (YYYY-MM-DD), `{id}` (series-form ID as per `filename_form`), `{slug}` (lowercase kebab-case title).

`heading_pattern_closed` / `heading_pattern_pending` — the template for the H2 heading of each file. Supported placeholders: `{date}`, `{id-dotted}`, `{id-undotted}`, `{title}`. The pending heading usually includes `⚠️` and a `— PENDING DECISION` suffix; keep both literally — downstream tooling matches them.

## First-run behaviour

If no `decisions.config.md` exists when either skill is invoked, the skill must ask the user for:

1. Which decision domain applies (free text, e.g. `db-architecture`).
2. Paths for the closed-decisions, pending-decisions (if any), and action-plans folders.
3. Which ID series to use, and for each series: the prefix, whether the ID appears in the heading, and in what form.

After the user answers, the skill offers to write a starter `decisions.config.md` with their answers so the next run is automatic. The user may decline — in which case the skill proceeds with the ad-hoc answers for this run only.

## Defaults when the user declines to configure

If the user refuses to create a configuration and does not supply paths, the skills fall back to these conservative defaults:

- Closed decisions: `decisions/` (no subfolder split — all closed decisions land directly here, regardless of `decision-type`).
- Pending decisions: `decisions-pending/`
- Action plans: `action-plans/`
- Single ID series: prefix `A`, `case: upper`, `heading_form: dotted`, `filename_form: undotted`.
- Index file: `_index.md` in each folder.
- `decision_types`: `[final, temporary]` are still recognised, and the `record-decision-made` skill still asks the user to pick one at closure time. Without `closed_subfolders` declared, both kinds of decisions co-exist in the same folder; only the `decision-type` frontmatter field distinguishes them.
- `revisit_trigger_schema`: the four built-in trigger types (`phase`, `date`, `threshold`, `external`) are accepted on `temporary` decisions even when no schema is configured.

These defaults are safe but rarely ideal — losing the visual separation between final and temporary decisions in a flat folder makes revisit-due decisions hard to spot. The skills should remind the user on each subsequent run that defaults are in use and a configuration with `closed_subfolders` would be cheaper.

## Migration

Workspaces that pre-date this configuration file can adopt it without moving existing files. Point the `folders` entries at the current folders. The skills will continue to use the existing paths; they never relocate existing artefacts automatically.
